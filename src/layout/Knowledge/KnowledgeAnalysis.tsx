...
export function KnowledgeCalls({ index, nodeId, direction, params }: { index: KnowledgeIndex; nodeId: string; direction: "callers" | "callees"; params: URLSearchParams }) {
  ...
  return <section className="knowledge-analysis"><div className="knowledge-section-heading"><h3>{t(`knowledge.${direction}`)}{data && ` · ${data.total.toLocaleString()}`}</h3><label>{t("knowledge.depth")} <select className="input" value={data?.depth ?? depth ?? ""} onChange={e => memoryNavigate(knowledgeUrl(index.projectId, { knowledgeCallDepth: e.target.value }))}>{data ? Array.from({ length: data.maxDepth }, (_, i) => i + 1).map(value => <option key={value}>{value}</option>) : <option value={depth || ""}>—</option>}</select></label></div>
...
export function KnowledgeImpact({ index, nodeId, params }: { index: KnowledgeIndex; nodeId: string; params: URLSearchParams }) {
  ...
  return <section className="knowledge-analysis"><div className="knowledge-section-heading"><h3>{t("knowledge.impact")}</h3><label>{t("knowledge.depth")} <select className="input" value={data?.depth ?? depth ?? ""} onChange={e => memoryNavigate(knowledgeUrl(index.projectId, { knowledgeDepth: e.target.value }))}>{!data ? <option value={depth || ""}>—</option> : Array.from({length:data.maxDepth},(_,i) => i+1).map(value => <option key={value}>{value}</option>)}</select></label></div>
