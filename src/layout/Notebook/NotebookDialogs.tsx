...
export function NotebookDialog({action,tree,overview,onChanged}:{action:string;tree:NotebookTree;overview:NotebookOverview;onChanged:()=>void}) {
  ...
  return <form className="nb-dialog" onSubmit={e=>{e.preventDefault();void submit();}}>
    ...
    {(action==="new"||action==="folder"||action==="import")&&<label>{t("nb.folder")}<select className="input" value={parent} onChange={e=>setParent(e.target.value)}><option value="">{tree.vault.name}</option>{tree.nodes.filter(n=>n.kind==="folder").map(n=><option key={n.path} value={n.path}>{n.path}</option>)}</select></label>}
